...
void OnSwapChainPrePresent(wrap_event_data* evd) { Direct3D11Loader::i().PrePresentSwapChain(); }

void OnSwapChainPrePresent1(wrap_event_data* evd) { Direct3D11Loader::i().PrePresentSwapChain(); }
...
void OnDXGIPostCreateSwapChain(wrap_event_data* evd) {
    ...
    LogDebug("Called OnDXGIPostCreateSwapChain(hwnd: {}, iunk: {}, device: {}, swc: {})", LogPtr | params.pDesc->OutputWindow,
             LogPtr | params.pDevice, LogPtr | dev, LogPtr | *params.ppSwapChain);
    if(dev)
...
void OnDXGIPostCreateSwapChainForHwnd(wrap_event_data* evd) {
    ...
    ID3D11Device* dev;
    GW2_CHECKED_HRESULT(params.pDevice->QueryInterface(&dev));
    LogDebug("Called OnDXGIPostCreateSwapChainForHwnd(hwnd: {}, iunk: {}, device: {}, swc: {})", LogPtr | params.hWnd,
             LogPtr | params.pDevice, LogPtr | dev, LogPtr | *params.ppSwapChain);
    if(dev)
...
void OnSwapChainPreResizeBuffers(wrap_event_data* evd) {
    LogDebug("Called OnSwapChainPreResizeBuffers(width: {}, height: {})", evd->stackPtr->ResizeBuffers.Width,
             evd->stackPtr->ResizeBuffers.Height);
    Direct3D11Loader::i().PreResizeSwapChain();
}

void OnSwapChainPreResizeBuffers1(wrap_event_data* evd) {
    LogDebug("Called OnSwapChainPreResizeBuffers1(width: {}, height: {})", evd->stackPtr->ResizeBuffers.Width,
             evd->stackPtr->ResizeBuffers.Height);
    Direct3D11Loader::i().PreResizeSwapChain();
}

void OnSwapChainPostResizeBuffers(wrap_event_data* evd) {
    LogDebug("Called OnSwapChainPostResizeBuffers(width: {}, height: {})", evd->stackPtr->ResizeBuffers.Width,
             evd->stackPtr->ResizeBuffers.Height);
...
void OnSwapChainPostResizeBuffers1(wrap_event_data* evd) {
    LogDebug("Called OnSwapChainPostResizeBuffers1(width: {}, height: {})", evd->stackPtr->ResizeBuffers.Width,
             evd->stackPtr->ResizeBuffers.Height);
    Direct3D11Loader::i().PostResizeSwapChain(evd->stackPtr->ResizeBuffers.Width, evd->stackPtr->ResizeBuffers.Height);
}
...
void Direct3D11Loader::PrePresentSwapChain() { GetBaseCore().Draw(); }
...
void Direct3D11Loader::Init(gw2al_core_vtable* gAPI) {
    ...
    d3d9_wrap.enable_event = static_cast<pD3D9_wrapper_enable_event>(
        gAPI->query_function(gAPI->hash_name(const_cast<wchar_t*>(D3D9_WRAPPER_ENABLE_EVENT_FNAME))));
